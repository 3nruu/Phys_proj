...
# Carefully chosen to keep screen at d²/λ within x_max = 12.0
# All combinations verified to stay under 12:
# Max: k=30, d=1.55 → x=11.48 ✓
kx0_values = [20.0, 25.0, 30.0]  # wave numbers
d_values = [1.4, 1.5, 1.55]  # slit separations (all stay < 12)
...
# Double slit parameters (y1, y2 will be set per simulation based on d)
...
def run_simulation(params):
    ...
    # Calculate screen position: d²/λ (Fresnel number = 1)
    x_screen_fixed = (d**2) / wavelength
    
    # Set slit positions based on d
    y1 = -d / 2
    y2 = d / 2
    
    print(f"Running: kx0={kx0:.1f}, d={d:.1f}, h={h:.1f}")
    print(f"  λ={wavelength:.3f}, screen at x={x_screen_fixed:.2f} (d²/λ, F=1)")
...
if __name__ == '__main__':
    ...
    n_rows = len(d_values) * len(h_values)
    ...
    for idx, result in enumerate(results):
        ...
        d_idx = d_values.index(d)
        h_idx = h_values.index(h)
        
        row = d_idx * len(h_values) + h_idx
        col = kx0_idx
    ...
    for d in d_values:
        for h in h_values:
            ...
            for result in results:
                if result['d'] == d and result['h'] == h:
                    errors.append(result['error_L2'])
                    kx0_plot.append(result['kx0'])
            if errors:
                ax1.plot(kx0_plot, errors, 'o-', lw=2, ms=6, label=f'd={d}, h={h}')
    ...
    for kx0 in kx0_values:
        for h in h_values:
            ...
            if errors:
                ax2.plot(d_plot, errors, 'o-', lw=2, ms=6, label=f'k={kx0}, h={h}')
    
    ax2.set_xlabel('Slit separation d')
    ax2.set_ylabel('L2 Error')
    ax2.set_title('Error vs Slit Separation (screen at d²/λ)')
    ax2.legend(fontsize=7)
    ax2.grid(True)
    
    # Error vs hole width h
    for kx0 in kx0_values:
        for d in d_values:
            errors = []
            h_plot = []
            for result in results:
                if result['kx0'] == kx0 and result['d'] == d:
                    errors.append(result['error_L2'])
                    h_plot.append(result['h'])
            if errors:
                ax3.plot(h_plot, errors, 'o-', lw=2, ms=6, label=f'k={kx0}, d={d}')
    
    ax3.set_xlabel('Hole width h')
    ax3.set_ylabel('L2 Error')
    ax3.set_title('Error vs Hole Width')
    ...
    for idx, result in enumerate(results):
        ...
        # Mark the screen position (F=1 by construction)
        ax.axvline(x_screen, color='red', ls='--', lw=1.5, alpha=0.7, label=f'screen (d²/λ)')
        
        ax.set_xlabel('x')
        ax.set_ylabel(r'$\|QM - Fresnel\|_{L^2(y)}$')
        ax.set_title(f'k={kx0:.1f}, d={d:.1f}, h={h:.1f}\nx={x_screen:.1f}', fontsize=7)
        ax.legend(fontsize=7)
        ax.grid(True, alpha=0.3)
        ax.set_xlim(0, x_max)
